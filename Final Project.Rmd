...
Cluster 1 has all developing countries (53) with a median Life Expectancy of around 58 years, mean logged population of 15.5 (~15M) people, an average of about 9 years of schooling and is mainly comprised of 38 countries in Africa, 13 in Asian, 1 in the Americas, and 1 in Oceana.

Cluster 2 has all developing countries (55) with a median Life Expectancy of around 73 years, a mean logged population of 14.09 (~14M) people, an average of about 12.5 years of schooling and is mainly comprised of22 countries in the Americas, 17 in Asia, 6 in Oceana, 5 in Africa, and 5 in Europe.

Cluster 3 has mostly developed countries (26) and 6 developing countries, with a median Life Expectancy of around 80 years, a mean logged population of 14.87 people (~ 14M), an average of almost 16 years of schooling and is mainly comprised of countries in Europe, 29, and Oceania, 1.
...
This clustering tree has a control for cross validation X-val, and Cost complexity parameter (cp) where we now have a vertical line at 6, in relation to our leaf nodes in the decision tree. Shown below.
...
To show the value of a 4 maximum node tree, we can see the plot below to review the diminishing returns as the tree continues to grow.
...
```
 ...
 The default bagging is 25 boostrap samples and trees, but we changed our metrics to review a broader scope with more samples. Here we can see that of the 100 tree samples used, the error more or less stableizes after around 30 tree samples are used.
```{r}
ntree <- 10:100
...
We can look at the variables used in the models to predict their importance and get a random mean squared error of 3.059, the lowest yet.
```{r}
...
Another algorithm: Gradient boosting: classification and regression. like AdaBoost, but use gradient descent to address the current errors. The method can overfit, so using cross-validation is very important (cv.folds). The default method uses gaussian.
