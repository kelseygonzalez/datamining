...
We find diminishing returns after 6 terminal nodes at a clost complexity of .012.
You may also notice the dashed line which goes through the terminal node, tree size= 5.
```{r}
plotcp(m1)
```

This clustering tree has a control for cross validation X-val, and Cost complexity where we now have a vertical line at 8, cutting the peak height.
```{r}
m2 <- rpart(
   formula = Life ~ .,
   data    = train,
...
Now we can test different models
```{r}
