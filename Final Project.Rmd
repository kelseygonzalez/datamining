...
```
 ...
 The default bagging is 25 boostrap samples and trees, but we changed our metrics to review a broader scope with more samples. Here we can see that of the 100 tree samples used, the error stableizes around 81 tree samples are used.
```{r}
ntree <- 10:100
...
abline(v=81, col = "blue", lty = "dashed")
...
```{r}
ct <- boosting(Life ~ ., train, mfinal=50)
ps2 <- predict(ct, test) 
ps2$confusion
ps2$error
```
