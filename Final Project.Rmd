...
data %>%
  ...
  geom_point(size = 3.5, 
             ...
             col = "white", 
             fill = "#0162B2") +  
  geom_text(data = subset(data, rlang::is_empty(data$label)), 
            aes(BMI, `Life expectancy`,label=Country))
...
h <- hclust(d = as.dist(gower_dist), method="complete")
  hc <- hclust(d = as.dist(gower_dist), method = "ward.D2")
  plot(hc, cex = 0.6, hang = -1, main="Hierarchal Cluster of Predicted Probabilities")
 
```

```{r}
data_dendro <- data2013 %>% select(-c(region, Status,Country)) %>% scale()
rownames(data_dendro) <- data2013$Country
data_dendro <- dist(data_dendro, method="manhattan")
data_dendro <- as.dendrogram(hclust(as.dist(gower_dist), method="ward.D2"))
