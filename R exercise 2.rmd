...
```{r}
hist(data2$GDP)
data2$gdpPercaplog = log(data2$gdpPercap)

plot(density(data2$gdpPercap))
lines(density(data2$gdpPercaplog), add = TRUE)
# they're on different scales, so you can't really see the differences. 

plot(density(data2$gdpPercaplog))
...
data2 %>% select(Diphtheria, dipnormal)
...
```{r qqplot=plot, echo = FALSE}
suppressPackageStartupMessages(library(car))
qqPlot(data2$`Life expectancy`, main = "Life Expectancy QQ Plot")
data2[48, "Population"] <-mean(data2$pop,  na.rm = TRUE)

```

We found that GDP correlates fully with Percentage Expenditure; Under 5 deaths correlates with infant deaths; and schooling and Income composition of resources is .9 correlated. 

```{r}
suppressPackageStartupMessages(library(DMwR2))
m = lm(data2$GDP~ data2$`percentage expenditure`, data=data2)
summary(m)
```
...
```{r}
data2$Over65 <- data2$`Life expectancy` > 65

```
#### Categorical classification of low, med, and high of Life Expectancy rates using 1/3 quartiles

```{r}
LifeExpectancy_low <-quantile(data2$`Life expectancy`, probs = c(1/3, 2/3), na.rm = TRUE)[1]
LifeExpectancy_high <-quantile(data2$`Life expectancy`, probs = c(1/3, 2/3), na.rm = TRUE)[2]
```

```{r}
data2$category <- "med"
data2$category [LifeExpectancy$`Life expectancy`< LifeExpectancy_low] <- "Low"
data2$category [LifeExpectancy$`Life expectancy`< LifeExpectancy_high] <- "High"
