...
convert("WB_pop_gdp.csv", "WB_pop_gdp.rda")
load('WB_pop_gdp.rda')
Time<-as.factor(x$Time)
as.numeric(x$Time)
```

```{r}
suppressPackageStartupMessages(require(dplyr))
data3 <-inner_join(data, x, by = c("Year" = "Time", "Country" = "Country Name"))
...
data = subset(data3,select = -c(`Hepatitis B`))
dplyr::glimpse(data3)

=======
>>>>>>> fa86954112854493fc6dfd8403f06e5b4fc5b3ec
...
year x country x life expectancy 

```{r}
test1  <-  subset(data2, select = c(Year, Country, `Life expectancy`))
unique(test1$Country)
unique(test1$Year)

# test2 = as.array(unlist(test1),dim=c(193,2,15))


# require(tidyr)
#test2 <- spread(test1, key="Year", value="Life expectancy")

# This is as far as I got. need to separate by year to make a cube.
```
...
year x country x life expectancy x status 
