...
Load packages
```{r}
<<<<<<< HEAD:R-exercises/R exercise 6.Rmd
...
library(ggdendro)
>>>>>>> 1d9768eaea87a915856b39d10bcd0e7192a45709:R exercise 6.Rmd
```
...
Plot silhouette width (higher is better, clusters = 2 or 3 has the highest sil value)
```{r}
plot(1:10, sil,
...
```
The results suggest
Cluster 1, is mainly developing countries in Africa 
Cluster 2, is mainly developing countries in the Americas and Asia
Cluster 3, is mainly developed countries in Europe 
...
2 or 4 clusters seem to be the best, with margin differences between them

```{r}
kmeans_fit <- kmeans(data_scaled[,-1], 4)
```
...
40+53+28 = 121 universities were in the same clusters(121/139 = 87% agreement)
```{r}
result2 <- data_scaled
...
If we want to cluster at a height of about 0.3...
```{r}
clus <- cutree(h, h=.3)
table(clus)
# or just 3 clusters...
...
table(clus3)
```
clus is a vector with the number of the cluster to which each observation was assigned. Observations keep the original order as in the dissimilar matrix.
...
```
the results aren't really consisent.
