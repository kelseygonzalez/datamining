...
suppressPackageStartupMessages(library(ggplot2))
ggplot(data = LifeExpectancy,
       mapping = aes(x = LifeExpectancy$Status,
                     y = LifeExpectancy$`Life expectancy`)) +
  geom_point() +
  facet_wrap(~ LifeExpectancy$Year)
```


```{r}
boxplot(formula = LifeExpectancy$`Life expectancy` ~ LifeExpectancy$Year, data = LifeExpectancy)
